...
void dsi_ctrl_hw_cmn_map_mdp_regs(struct platform_device *pdev,
				  struct dsi_ctrl_hw *ctrl)
{
	if (!ctrl->mdp_intf_base)
		return;

	ctrl->line_count_reg = ctrl->mdp_intf_base + 0x0B0;
}

u32 dsi_ctrl_hw_cmn_log_line_count(struct dsi_ctrl_hw *ctrl, u32 mode)
{
	if (!ctrl->line_count_reg)
		return 0;

	return readl(ctrl->line_count_reg);
}
